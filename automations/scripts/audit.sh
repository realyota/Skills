...
case "$MODE" in
    all)
        PROMPT_FILE="$PROMPTS_DIR/audit-all.md"
        ;;
    conditional)
        PROMPT_FILE="$PROMPTS_DIR/audit-conditional.md"
        ;;
    *)
        echo "Unknown AUDIT_MODE: $MODE (use all|conditional)" >&2
        exit 1
        ;;
esac
